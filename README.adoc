...
These Maven properties are then passed to the Java test program as the [hotspot=192-196 file=1]`<systemPropertyVariables/>` element in
...
The first thing to check is that a `200` response was received. The JUnit [hotspot=47-48 file=0]`assertEquals` method can
...
for both JVMs is the same. You could write additional assertions [hotspot=47-48 file=0]`assertEquals` to check for more values.
