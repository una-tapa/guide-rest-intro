...
このガイドでは、JAX-RSとJSON-Pを使って簡単なRESTサービスを構築し、テストする方法を学びます。このサービスは、`\http://localhost:9080/LibertyProject/System/properties`への`GET`リクエストに応答して、JVMのシステムプロパティを表示するものです。
...
新しいRESTアプリケーションを作成するときには、きちんとAPIの設計をすることが重要です。 JAX-RS APIは、JSON-RPC、またはXML-RPC APIを作成するためにも使用できますが、それはRESTfulなサービスではありません。良いRESTfulサービスは、サービスによって公開されているリソース（このガイドの例では、システムプロパティがリソースとなります。）を、作成、読み取り、更新、削除できるように設計されています。

サービスは `/System/properties`というパスへの`GET`リクエストに対して、`200 OK`と共に、JVMのすべてのシステムプロパティを返します。
