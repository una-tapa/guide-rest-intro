...
The [hotspot=19-21]`SystemApplication` class extends the `Application` class, which in turn associates all JAX-RS
resource classes in the WAR file with this JAX-RS application, making them available under the common
path specified in the [hotspot=19-21]`SystemApplication` class. The [hotspot=18]`@ApplicationPath` annotation has a
...
The [hotspot=24 file=0]`@Path` annotation on the class indicates that this resource responds to the `properties` path
in the JAX-RS application. The [hotspot=18 file=1]`@ApplicationPath` annotation in the [hotspot=19-21 file=1]`SystemApplication` class together with
the [hotspot=24 file=0]`@Path` annotation in this class indicates that the resource is available at the `System/properties`
path.
...
The [hotspot=29 file=0]`@GET` annotation on the method indicates that this method is to be called for the HTTP `GET`
method. The [hotspot=30 file=0]`@Produces` annotation indicates the format of the content that will be returned, the
value of the [hotspot=30 file=0]`@Produces` annotation will be specified in the HTTP `Content-Type` response header.
...
. Creates a [hotspot=33 file=0]`JsonObjectBuilder` object using the `Json` class. The [hotspot=33 file=0]`JsonObjectBuilder` is then used
to populate a `JsonObject` with values.
. Calls the [hotspot=35 file=0]`getProperties` method on the `System` class to get a `Properties` object that contains
all the system properties.
. Calls the [hotspot=36 file=0]`entrySet` method on the `Properties` object to get a `Set` of all the entries.
. Convert the `Set` to a `Stream` (new in Java SE 8) by calling the [hotspot=37 file=0]`stream` method. Streams make
working through all the entries in a list very simple.
. Calls the [hotspot=38-39 file=0]`forEach` method on the `Stream` passing in a function that will be invoked for each
entry in the `Stream`. The function passed in will call the [hotspot=38-39 file=0]`add` method on the [hotspot=33 file=0]`JsonObjectBuilder`
for every entry in the stream. The key and value for the `JsonObject` will be obtained by calling
the [hotspot=38 file=0]`getKey` and [hotspot=39 file=0]`getValue` methods on the `Map.Entry` objects in the stream.
. Returns the `JsonObject` by calling the [hotspot=41 file=0]`build` method on the [hotspot=33 file=0]`JsonObjectBuilder`.
...
The test method is indicated with the [hotspot=28 file=0]`@Test` annotation.
...
file. Refer to the [hotspot file=1]`pom.xml` file to see how the application information such as the [hotspot=23 file=1]`<app.name/>`, [hotspot=24 file=1]`<testServerHttpPort/>` and [hotspot=25 file=1]`<testServerHttpsPort/>` elements are provided in the file.
...
Getting the values to create a representation of the URL is simple. The test class uses the [hotspot=31-32 file=0]`getProperty` method
...
class registered with it by calling the [hotspot=38 file=0]`register` method and providing the `Class` object for the
`JsrJsonpProvider` class.

To call the JAX-RS service using the JAX-RS client, you first create a `WebTarget` object by calling
the [hotspot=42 file=0]`target` method providing the URL. To cause the HTTP request to occur first the [hotspot=43 file=0]`request` method
on `WebTarget` and then the [hotspot=43 file=0]`get` method on the returned object need to be called. The [hotspot=43 file=0]`get` method
call is a synchronous call that blocks until a response is received. This call returns a [hotspot=43 file=0]`Response`
object, which can be interrogated to determine whether the request was successful.

The first thing to check is that a `200` response was received. The JUnit [hotspot=response file=0]`assertEquals` method can
...
for both JVMs is the same. You could write additional assertions [hotspot=response file=0]`assertEquals` to check for more values.
