...
JAX-RSには、REST APIを作成するための2つの重要な概念があります。１つめの概念は、リソース自体で、クラスとしてモデル化されています。２つ目の概念はJAX-RSアプリケーションです。JAX-RSアプリケーションが、公開されたすべてのリソースを共通のパスの下にグループ化します。 JAX-RSアプリケーションは、すべてのリソースのラッパー（操作を扱いやすくする仕組み）として考えることができます。普通は、複数のリソースに対して、１つのJAX-RSアプリケーションを作るのが一般的ですが、複数のJAX-RSアプリケーションを作成することも可能です。
...
このリソースクラスにはかなりのコードが含まれているので、少しずつ見ていきましょう。

このクラスの中の [hotspot=11 file=0]`@ Path`　アノテーションは、このリソースがJAX-RSアプリケーションで　`properties`　パスに応答することを示しています。 [hotspot = 7-9 file = 1]`SystemApplication` クラスの [hotspot = 6 file = 1]`@ApplicationPath` アノテーションと一緒に
このクラスの [hotspot = 11 file = 0] `@Path`アノテーションはリソースが`System/properties` のパスで利用可能にします。
